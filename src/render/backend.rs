...
impl Backend {
    pub fn new() -> Self{
        ...
        let context_attribs = [
            glx::GLX_RGBA, glx::GLX_DEPTH_SIZE, 24, glx::GLX_DOUBLEBUFFER, 0
        ];
        unsafe{
            let display = xlib::XOpenDisplay(ptr::null());
            let root = xlib::XDefaultRootWindow(display);
            let visual = glx::glXChooseVisual(display, 0, context_attribs.as_ptr() as *mut _);
            let context = glx::glXCreateContext(display, visual, 0 as glx::GLXContext, true as xlib::Bool);
            let fb_config = glx::glXChooseFBConfig(display, xlib::XDefaultScreen(display), visual_attribs.as_ptr(), &mut fbcount);
    ...
    pub fn xr_session_create_info(&self) -> xr::opengl::SessionCreateInfo{
        let visualid = unsafe{
            {*self.visual}.visualid as u32
        };
